...
for i in tqdm(range(0,221,20)):
    ...
    with open('frame'+text[-4:]+'.obj', 'r') as file:
        ...
        data[i] = [count,np.mean(faces['pomer_polomerov']),np.mean(faces['pomer_extrem_stran']),np.mean(faces['pomer_polomer_polobvod']),np.mean(faces['pomer_stran']),np.mean(faces['pomer_opisana_strana']),np.mean(faces['obsahy'])]    
...
df.index.name = 'Index'
df = df.rename(columns={0: 'Pocet', 1: 'pomer_polomerov', 2: 'pomer_extrem_stran', 3: 'pomer_polomer_polobvod', 4 : 'pomer_stran', 5 : 'pomer_opisana_strana', 6 : 'obsahy'})
print(df)
df.to_csv('data1.csv')
